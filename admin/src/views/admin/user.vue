            ...
            <ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
              <li>
                <a href="#" class="tooltip-success" data-rel="tooltip" title="Edit">
																			<span class="green">
																				<i v-on:click="edit(user)" class="ace-icon fa fa-pencil-square-o bigger-120"></i>
																			</span>
                </a>
              </li>
...
<script>
  ...
  export default {
    ...
    mounted: function (){
      ...
      _this.list(1);
      //sidebar激活样式方法一
      // this.$parent.$parent.activeSidebar("system-user-sidebar");
    },
    methods:{
      /**
       * 查找是否有权限
      ...
      del(id){
        ...
        Confirm.show("删除用户后不可恢复,确认删除?",function (){
          Loading.show();
          _this.$axios.delete(process.env.VUE_APP_SERVER+'/system/admin/user/delete/'+id).then((response)=>{
            Loading.hide();
            let resp = response.data;
            if (resp.success){
              _this.list(1);
      ...
      savePassword() {
        ...
        _this.user.password = hex_md5(_this.user.password + KEY);
        Loading.show();
        _this.$axios.post(process.env.VUE_APP_SERVER + '/system/admin/user/save-password', _this.user).then((response)=>{
          Loading.hide();
          let resp = response.data;
          if (resp.success) {
