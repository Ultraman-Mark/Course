            ...
            <ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
              <li>
                <a href="#" class="tooltip-success" data-rel="tooltip" title="Edit">
																			<span class="green">
																				<i v-on:click="edit(file)" class="ace-icon fa fa-pencil-square-o bigger-120"></i>
																			</span>
                </a>
              </li>
  ...
  export default {
    ...
    methods:{
      ...
      save(){
        ...
        Loading.show();
        _this.$axios.post(process.env.VUE_APP_SERVER+'/file/admin/file/save',_this.file).then((response)=>{
          Loading.hide();
          let respd = response.data;
          if (respd.success){
      ...
      del(id){
        ...
        Confirm.show("删除文件后不可恢复,确认删除?",function (){
          Loading.show();
          _this.$axios.delete(process.env.VUE_APP_SERVER+'/file/admin/file/delete/'+id).then((response)=>{
            Loading.hide();
            let respd = response.data;
            if (respd.success){
              _this.list(1);
