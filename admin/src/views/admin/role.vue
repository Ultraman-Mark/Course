            ...
            <ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
              <li>
                <a href="#" class="tooltip-success" data-rel="tooltip" title="Edit">
																			<span class="green">
																				<i v-on:click="edit(role)" class="ace-icon fa fa-pencil-square-o bigger-120"></i>
																			</span>
                </a>
              </li>
...
<script>
  ...
  export default {
    ...
    mounted: function (){
      ...
      _this.list(1);
      //sidebar激活样式方法一
      // this.$parent.$parent.activeSidebar("system-role-sidebar");
    },
    methods:{
      /**
       * 新增
      ...
      save(){
        ...
        Loading.show();
        _this.$axios.post(process.env.VUE_APP_SERVER+'/system/admin/role/save',_this.role).then((response)=>{
          Loading.hide();
          let resp = response.data;
          if (resp.success){
      ...
      del(id){
        ...
        Confirm.show("删除角色后不可恢复,确认删除?",function (){
          Loading.show();
          _this.$axios.delete(process.env.VUE_APP_SERVER+'/system/admin/role/delete/'+id).then((response)=>{
            Loading.hide();
            let resp = response.data;
            if (resp.success){
              _this.list(1);
      ...
      loadResource() {
        let _this = this;
        Loading.show();
        _this.$axios.get(process.env.VUE_APP_SERVER + '/system/admin/resource/load-tree').then((res)=>{
          Loading.hide();
      ...
      listRoleResource() {
        let _this = this;
        _this.$axios.get(process.env.VUE_APP_SERVER + '/system/admin/role/list-resource/' + _this.role.id).then((response)=>{
          let resp = response.data;
      ...
      listRoleUser() {
        let _this = this;
        _this.roleUsers = [];
        _this.$axios.get(process.env.VUE_APP_SERVER + '/system/admin/role/list-user/' + _this.role.id).then((res)=>{
          let response = res.data;
